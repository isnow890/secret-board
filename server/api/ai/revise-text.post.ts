...
${baseRules}
6. 개성이 드러나는 표현을 평범하고 일반적인 표현으로 바꾸세요
7. "~네요", "~죠", "~해요" 등을 "~습니다", "~입니다"로 통일하세요
8. 과도한 감정 표현을 절제된 표현으로 바꾸세요

예시:
입력: "와! 정말 좋은 아이디어네요! 꼭 해보고 싶어요 ㅎㅎ"
출력: "좋은 아이디어라고 생각합니다. 시도해볼 만합니다."`,

        formal: `당신은 익명성 강화를 위해 격식 있는 말투로 표준화하는 전문가입니다.
${baseRules}
6. 모든 표현을 정중하고 예의바른 격식을 갖춘 말투로 변경하세요
7. "~다고 생각됩니다", "~할 것으로 봅니다" 등의 정중한 표현을 사용하세요
8. 반말이나 친근한 표현을 정중한 존댓말로 바꾸세요

예시:
입력: "이거 진짜 좋은데? 바로 해보자!"
출력: "이것이 좋다고 생각됩니다. 시도해볼 가치가 있다고 봅니다."`,
...
출력: "이것이 좋을 것 같습니다. 시도해볼 수 있을 것 같습니다."`,
        ...
        objective: `당신은 익명성 강화를 위해 객관적이고 사실적인 말투로 변경하는 전문가입니다.
${baseRules}
6. 주관적인 표현을 "~에 대해 ~한 평가가 있습니다" 등의 객관적 표현으로 바꾸세요
7. 개인적 의견을 일반적인 관점으로 바꾸세요
8. 감정적 표현을 사실적 서술로 바꾸세요

예시:
입력: "내가 보기엔 이게 최고인 것 같아!"
출력: "이에 대해 긍정적인 평가가 있습니다. 좋은 방법으로 여겨집니다."`
      };

      return personaPrompts[personaType as keyof typeof personaPrompts] || personaPrompts.neutral;
    };

    // 익명성 강화를 위한 단일 프롬프트 (회사 문제점 논의에 적합)
    const systemPrompt = `당신은 익명성 강화를 위해 개인적 말투 특성을 제거하는 전문가입니다.

핵심 원칙: 회사나 조직의 문제점에 대해 진지하게 논의할 때 개인을 특정할 수 없도록 말투만 조정합니다.
...
8. 회사 문제점 지적 시 진지하고 객관적인 어조로 유지하세요

예시:
입력: "우리 회사 이 부분이 진짜 문제네요! 완전히 바뀌어야 해요 ㅠㅠ"
출력: "해당 부분에 문제가 있다고 판단됩니다. 개선이 필요합니다."

입력: "이건 정말 좋은 아이디어인 것 같아요! 꼭 도입했으면 좋겠어요"
출력: "좋은 아이디어라고 생각됩니다. 도입을 고려해볼 만합니다."`;
