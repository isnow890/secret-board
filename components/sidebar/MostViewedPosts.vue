<!-- components/sidebar/MostViewedPosts.vue -->
<template>
  <div class="space-y-3">
    <!-- 로딩 상태 -->
      ...
      >
        <div class="p-3">
          <h4
            ...
            :class="isViewed(post.id) ? 'text-text-tertiary group-hover:text-text-secondary' : 'text-text-primary group-hover:text-primary-500'"
          >
            {{ post.title }}
          </h4>
    ...
    <div v-else class="text-center py-4">
      ...
      />
      <p class="text-xs text-text-tertiary">게시글이 없습니다</p>
    </div>
  </div>
</template>
...
const loadMostViewedPosts = async () => {
  try {
    loading.value = true;
    error.value = "";

    const fetchedPosts = await fetchMostViewedPosts(5);
    posts.value = fetchedPosts;
    updateViewedPosts();
  } catch (err: any) {
    console.error("가장 많이 본 글 로드 실패:", err);
